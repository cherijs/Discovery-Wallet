...
App.cancelPayment = (args) => {
  ...
  App.changedInvoiceInput();

  return;
};

/** Changed invoice input
*/
App.changedInvoiceInput = e => {
  const invoiceInput = $('.preparing-send .request');
  ...
  if (/^lightning:\/\//.test(invoice)) {
    invoiceInput.val(invoice.slice('lightning://'.length));

    return App.changedInvoiceInput();
  } else if (/^lightning:/.test(invoice)) {
    invoiceInput.val(invoice.slice('lightning:'.length));

    return App.changedInvoiceInput();
  }

  App.is_decoding_invoice = true;
  ...
  return App.getDecodedInvoice({invoice}, (err, res) => {
    ...
    App.is_decoding_invoice = false;

    // Exit early if assumed state is changed
    if (invoice !== $('.preparing-send .request').val()) {
      return;
    }
    ...
    $('.payment-preview').data('invoice', invoice);
...
App.claimWelcomeGift = (args, cbk) => {
  ...
  $.post(`/v0/gifts/${args.wallet_id}/claim/welcome`)
  .done((res) => {
...
/** Clicked create new store button
*/
App.clickedCreateNewStore = (e) => {
  e.preventDefault();

  setTimeout(() => {
    $('html, body').animate({
      scrollTop: $('.card.store-creator').offset().top,
    },
  ...
  App.presentStoreCreator({});

  return;
};

/** Clicked delete store button
*/
App.clickedDeleteStore = function(e) {
  const button = $(this);
  ...
  return App.makeRequest({
    api: `stores/update/`,
    json: {id: storeId, is_deleted: true, wallet_id: App.wallet_id},
  },
  (err) => {
    button.removeClass('disabled').prop('disabled', false);

    if (!!err) {
      return App.presentError({err});
    }

    const card = $('.network-info .store');


    card.hide(App.animation_time_ms, () => {
      card.prop('hidden', true);

      App.switchToNetworkTab({});
...
App.createChainInvoice = (args, cbk) => {
  ...
  $.ajax({
    ...
    type: 'POST',
    url: `/v0/invoices/chain/${App.wallet_id}/`,
  })
  .done((res) => {
...
/** Create a new invoice
  ...
  {
    id: <Id String>
    invoice: <BOLT 11 Encoded Invoice String>
  }
*/
App.createInvoice = (args, cbk) => {
  $.ajax({
    ...
    type: 'POST',
    url: `/v0/invoices/${App.wallet_id}/`,
  })
  .done(res => {
    if (!res || !res.id || !res.invoice) {
      return cbk([500, 'Expected id, invoice', res]);
    }
...
*/
App.createStore = (args, cbk) => {
  if (!Array.isArray(args.products) || !args.products.length) {
    return cbk([0, 'Expected products']);
  }

  if (!args.title) {
    return cbk([0, 'Expected title']);
  }

  return App.makeRequest({
    api: `stores/create/`,
    json: {
      products: args.products.map((n) => {
        return {fiat_price: n.fiat_price, title: n.title};
      }),
      title: args.title,
  ...
  (err, res) => {
    ...
    if (!res || !res.id) {
      return cbk([503, 'Expected store response', res]);
    }

    return cbk(null, {id: res.id});
  });
};
...
App.getBalance = (args, cbk) => {
  $.get(`/v0/balances/${args.wallet_id}`, (res) => {
...
App.getDecodedInvoice = (args, cbk) => {
  return $.get(`/v0/decoded/invoice/${args.invoice}`, (r) => {
...
App.getSwapRate = (args, cbk) => {
  ...
  return $.get(`/v0/swaps/rate/${args.currency_code}`, (res) => {
...
App.init = (args) => {
  ...
  $('.create-new-store').click(App.clickedCreateNewStore);
  $('.delete-store').click(App.clickedDeleteStore);
  ...
  $('.preparing-send .request').on('input', App.changedInvoiceInput);
...
App.pastedInPreparingReceive = (e) => {
  ...
  const invoicePrefix = 'ln';
  const pasteData = e.originalEvent.clipboardData.getData('text');

  if (pasteData.substring(0, invoicePrefix.length) !== invoicePrefix) {
    return;
  }
...
/** Present altcoin swap store

  {}
*/
App.presentAltcoinSwap = () => {
  const card = $('.altcoin-swap.template').clone();
  const currencyCode = 'tLTC';

  $('.altcoin-swap.presented').remove();
  $('.transactions-break').after(card);

  card.addClass('presented').removeClass('template');
  card.hide();
  card.show(App.animation_time_ms);

  card.find('.rate').text('Getting swap rate...');

  App.getSwapRate({currency_code: currencyCode}, (err, res) => {
    if (!!err) {
      card.hide(App.animation_time_ms);
    ...
    const min = (res.min_swap_tokens / 1e8).toFixed(8);

    card.find('.amount, .chain-address').prop('disabled', false);
    card.find('.amount, .chain-address').removeClass('disabled');
    card.find('.amount').prop('max', (res.max_swap_tokens / 1e8));
    card.find('.amount').prop('min', (res.min_swap_tokens / 1e8));

    const unitConversionLabel = `(1 ${currencyCode} = $${res.fiat_value})`;

    card.find('.rate').text(unitConversionLabel);

    card.find('.amount').on('input', () => {
      const amount = card.find('.amount').val();

      const amountConverted = (parseFloat(amount) * 1e8 / res.rate).toFixed(8);

      const label = !amount ? '' : `${amountConverted} tLTC`;

      card.find('.rate').text(`${label} ${unitConversionLabel}`);
  ...
  card.find('.withdraw-cancel').click((e) => {
    e.preventDefault();
  ...
  card.find('.altcoin-swap').submit((e) => {
    e.preventDefault();

    const amount = card.find('.amount').val();

    const tokens = parseInt((parseFloat(amount) * 1e8).toFixed(), 10);
    const withdrawalAddress = card.find('.chain-address').val().trim();

    // Exit early when there is no withdrawalAddress
    ...
    card.find(disableElements.join(',')).prop('disabled', true);
    card.find(disableElements.join(',')).addClass('disabled');

    card.find('.withdraw-button').text(`Swapping tBTC to ${currencyCode}...`);
    ...
    (err, payment) => {
      card.find(disableElements.join(',')).prop('disabled', false);
      card.find(disableElements.join(',')).removeClass('disabled');

      card.find('.withdraw-button').text(`Withdraw to ${currencyCode}`);
...
App.presentSentPayment = (args) => {
  const hopLabel = args.hop_count - 1 > 1 ? 'Relays' : 'Relay';
  ...
  const fiatAmount = args.fiat_value;
  const hops = `${args.hop_count - 1} Lightning ${hopLabel}`;
  ...
  if (!!args.payment_secret) {
    card.find('.payment-secret').text(args.payment_secret);
...
/** Present store creator

  {}
*/
App.presentStoreCreator = (args) => {
  const card = $('.store-creator.template').clone();

  const addProduct = card.find('.add-product');
  const createButton = card.find('.create-store');
  const maxStoreItems = 9;
  const products = {};
  const storeListing = card.find('.store-listing');
  const storeTitle = card.find('.store-title');

  $('.store-creator.presented').remove();
  $('.transactions-break').after(card);
  ...
  storeTitle.on('input', () => {
    const title = storeTitle.val().trim();

    if (!title || !addProduct.prop('hidden')) {
      return;
    }

    addProduct.prop('hidden', false);
    addProduct.hide();
    addProduct.show(App.animation_time_ms);
  ...
  addProduct.submit((e) => {
    ...
    // Exit early when adding would exceed the product maximum limit
    if (Object.keys(products).length >= maxStoreItems) {
      return;
    }
    ...
    const product = {
      id: App.uuid({}),
      fiat_price: parseInt((parseFloat(price.val(), 10) * 100).toFixed(), 10),
      title: title.val().trim(),
    };

    if (!product.fiat_price || !product.title) {
      return;
    }

    const row = card.find('.product.template').clone();

    row.addClass('presented').removeClass('template');
    row.data('product_id', product.id);

    row.find('.remove').click((e) => {
      e.preventDefault();

      delete products[row.data().product_id];

      row.hide(App.animation_time_ms, () => row.remove());

      const hasNoProducts = !Object.keys(products).length;

      createButton.toggleClass('disabled', hasNoProducts);
      createButton.prop('disabled', hasNoProducts);
      storeListing.prop('hidden', hasNoProducts);
    ...
    [price, title].forEach((n) => n.val(''));

    card.find('.products').prepend(row);
    storeListing.prop('hidden', false);

    row.find('.price').text(`$${(product.fiat_price / 100).toFixed(2)}`);
    row.find('.title').text(product.title);

    products[product.id] = product;

    row.hide();
    row.show(App.animation_time_ms);
  ...
  card.find('.cancel').click((e) => {
    e.preventDefault();

    card.hide(App.animation_time_ms, () => card.remove());
  ...
  createButton.click((e) => {
    ...
    createButton.text('Creating Store...');

    $('.presented.wallet-error').remove();

    return App.createStore({
      products: Object.keys(products).map((id) => products[id]),
      title: storeTitle.val().trim(),
    },
    (err, store) => {
      if (!!err) {
        createButton.prop('disabled', false);
        createButton.removeClass('disabled');
        createButton.text('Create Store');

        return App.presentError({err});
      }

      $('.payment-actions .create-store').prop('hidden', true);
...
App.refreshInvoices = (args, cbk) => {
  ...
  return async.eachSeries(ids, (id, cbk) => {
    ...
    return $.get(`/v0/invoices/${App.wallet_id}/${id}`, (res) => {
...
App.sendPayment = (args, cbk) => {
  $.post(`/v0/payments/${args.wallet_id}/${args.invoice}`)
  .done((payment) => {
    let description = payment.description || '';
...
App.startWebSocket = (args) => {
  App.ws = new WebSocket(location.origin.replace(/^http/, 'ws'));
...
App.submitRequestPayment = (e) => {
  ...
  return App.createInvoice({tokens}, (err, res) => {
    ...
    card.find('.payment-request').val(res.invoice);
    card.find('.payment-amount').prop('href',
      `lightning:${res.invoice}`);
    ...
    card.find('.on-chain')
      .prop('href', `${swapsService}?invoice=${res.invoice}&network=testnet`);
...
App.submitSendPayment = (e) => {
  ...
  const lightningInvoice = $('.payment-preview').data().invoice;
  ...
  return App.sendPayment({
    invoice: $('.payment-preview').data().invoice,
    wallet_id: App.wallet_id,
  },
  (err, payment) => {
...
App.switchToNetworkTab = (args) => {
  ...
  $.get(`/v0/stores/list/${App.wallet_id}/`)
  .done((stores) => {
    ...
    card.prop('hidden', !stores.length);
    $('.create-store').prop('hidden', !!stores.length);
    ...
    const items = store.product_count === 1 ? 'item' : 'items';

    const productCountLabel = `${store.product_count} ${items} on sale.`;

    card.find('.delete-store').data({store_id: store.id});
    card.find('.show-store').prop('href', `/store/${store.id}/`);
    card.find('.store-title').text(store.title);
    card.find('.product-count').text(productCountLabel);
    card.show(App.animation_time_ms);

    return;
  })
  .fail((r) => App.presentError({err: [r.status, r.statusText]}));
